...
import { fetchAllMovie, fetchMoreCompleteAnime } from '../lib/api';
import ShowBigCard from '../components/ShowBigCard';
import MoviePageSkeletonLoader from '../components/loader/MoviePageSkeletonLoader';

// Add the fetchGenresWithPosters function to lib/api.js
export async function fetchGenres() {
  try {
    // First get all anime data
    const { animeList } = await fetchAllMovie();
...
function MoviePage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [animeData, genresData] = await Promise.all([
          fetchAllMovie(3), // Only load first 3 pages initially
          fetchGenres(),
        ]);
  ...
  if (loading) {
    return <MoviePageSkeletonLoader />;
  }
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-10">
          ...
          <div className="text-center text-gray-500 text-sm mt-4">
            Menampilkan {sortedShows.length} Movies
          </div>
        </div>
      </div>
    </div>
